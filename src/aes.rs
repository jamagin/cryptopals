...
// fn mix_columns(state: &mut u128) {
//     unsafe {
//         asm!("aesmc {x}, {x}", x = inout(reg) *state);
//     }
// }

// fn inv_mix_columns(state: &mut u128) {
//     unsafe {
//         asm!("aesimc {x}, {x}", x = inout(reg) *state);
//     }
// }
...
fn key_expansion(key: &[u8], n_k: usize, n_r: usize) -> Vec<u32> {
    ...
    for i in 0..n_k {
        w[i] = u32::from_le_bytes(key[(i * 4)..=(i * 4 + 3)].try_into().expect("key length"));
    }
...
mod test {
    use std::{
        arch::aarch64::{uint8x16_t, vaesimcq_u8, vaesmcq_u8},
        mem::transmute,
    };
    ...
    fn test_key_expansion() {
        ...
        let expanded: Vec<u32> = ref_data.iter().map(|x| u32::from_be(*x)).collect();
    ...
    #[test]
    fn test_mix_columns() {
        let input: [u8; 16] = Vec::from_hex_byte_array(b"1234567800000000ffffffff10101010")
            .unwrap()
            .try_into()
            .unwrap();
        unsafe {
            let input_native = transmute(input);
            let mixed = vaesmcq_u8(input_native);
            let unmixed = vaesimcq_u8(mixed);
            assert_eq_hex!(
                transmute::<uint8x16_t, [u8; 16]>(unmixed),
                transmute::<uint8x16_t, [u8; 16]>(input_native)
            );
        }
    }
